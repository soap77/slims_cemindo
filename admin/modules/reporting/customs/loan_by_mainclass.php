...
<?php
    ...
    // generate by classification group
    for($i=0; $i<=9; $i++) {           
        $output .= '<tr><td class="'.$row_class.'"><strong style="font-size: 1.5em;">'.$i.'00</strong></td>';
        $xlsrows[$xls_rc][$xls_cc] = $i.'00';
        $xls_cc++;
        ...
        foreach ($months as $month_num => $month) {
            $loan_q = $dbs->query("SELECT COUNT(*) FROM biblio AS b
                ...
                WHERE LEFT(b.classification, 1) = '$i' AND l.loan_date LIKE '$selected_year-$month_num-%'".( !empty($coll_type)?" AND i.coll_type_id=$coll_type":'' ));
    ...
    $output .= '<tr>';
    $output .= '<td class="dataListHeaderPrinted">'.__('Total Pinjaman/bulan').'</td>';
    foreach ($months as $month_num => $month) {
