        ...
        while (!feof($file)) {
            ...
            if ($row_count < $record_offset) {
                ...
                if ($field) {
                    ...
                    //$mpasswd = password_hash($field[20], PASSWORD_BCRYPT);
                    ...
                    $sql_str = "INSERT IGNORE INTO member
                        ...
                        expire_date, birth_date, member_notes, input_date, last_update)
                            VALUES ($member_id, $member_name, $gender,
                            $member_type_id, $member_email, $member_address, $postal_code,
                            $inst_name, $is_new,
                            $member_image, $pin, $member_phone,
                            $member_fax, $member_since_date, $register_date,
                            $expire_date, $birth_date, $member_notes, $curr_datetime, $curr_datetime)";
                    // send query
                    @$dbs->query($sql_str);
                    if (!$dbs->error) {
                        $inserted_row++;
