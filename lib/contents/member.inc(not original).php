...
?>
    ...
    <?php if ($sysconf['captcha']['member']['enable']) { ?>
      ...
      <!-- <div><input type="text" name="captcha_code" io"captcha-form" style="width: 80%;" /></div> -->
    <?php
...
<?php
    ...
    function changePassword()
    {
        ...
        $_form .= '<tr>'."\n";
        $_form .= '<td class="key alterCell" width="20%"><strong>'.__('Member ID').'</strong></td>';
        $_form .= '<td class="value alterCell2"><input type="text" name="member_id" /></td>';
        $_form .= '</tr>'."\n";
        $_form .= '<tr>'."\n";
        $_form .= '<td class="key alterCell" width="20%"><strong>'.__('Name').'</strong></td>';
        $_form .= '<td class="value alterCell2"><input type="text" name="name" /></td>';
        $_form .= '</tr>'."\n";
        $_form .= '<tr>'."\n";
     ...
     */
    function procChangePassword($str_curr_pass, $str_new_pass, $str_conf_new_pass, $memberName, $memberID)
    {
        global $dbs;
        // get hash from db
    ...
    function showMemberDetail()
    {
        ...
        $_detail .= '</table>'."\n";
        /*$_detail .= '<tr>'."\n";
        $_detail .= '<button type="submit" name="renewPass" class="btn btn-success"><a href="?p=news">' . __('Update Data') . '</a></button>';
        $_detail .= '</tr>'."\n";*/
    ...
    if (isset($_POST['changePass']) && $sysconf['auth']['member']['method'] == 'native') {
        $change_pass = procChangePassword($_POST['currPass'], $_POST['newPass'], $_POST['newPass2'], $_POST['member_id'], $_POST['name']);
        if ($change_pass === true) {
            $info = '<span style="font-size: 120%; font-weight: bold;">'.__('Your password have been changed successfully.').'</span>';
        } else {
            if ($change_pass === CURR_PASSWD_WRONG) {
	...
	echo '<div class="tagline">';
        echo '<div class="memberInfoHead">'.__('Update Data').'</div>'."\n";
	    echo '</div>';
        echo changePassword();
    }
    ?>
